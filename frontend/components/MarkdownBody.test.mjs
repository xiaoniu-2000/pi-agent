...
test("renders local file links with preview and download controls", () => {
  const html = renderMarkdown("[file](components/MarkdownBody.tsx)");

  assert.match(html, /class="artifact-link-open"/);
  assert.match(html, /<span>file<\/span>/);
  assert.match(html, /href="\/api\/files\/home\/me\/project\/components\/MarkdownBody\.tsx\?type=download"/);
  assert.match(html, /download="MarkdownBody\.tsx"/);
  assert.doesNotMatch(html, /target=|rel=|\snode=/);
});

test("adds the managed session id to generated image and download URLs", () => {
  const html = renderToStaticMarkup(
    React.createElement(MarkdownBody, {
      cwd: "/data/users/user1/sessions/session-1/workspace",
      onOpenFile() {},
      sessionId: "session-1",
    }, "![plot](plot.png)\n\n[report](report.docx)"),
  );

  assert.match(html, /src="\/api\/files\/data\/users\/user1\/sessions\/session-1\/workspace\/plot\.png\?type=read&amp;sessionId=session-1"/);
  assert.match(html, /href="\/api\/files\/data\/users\/user1\/sessions\/session-1\/workspace\/report\.docx\?type=download&amp;sessionId=session-1"/);
});
