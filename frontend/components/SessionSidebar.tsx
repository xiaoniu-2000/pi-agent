...
export function SessionSidebar({ selectedSessionId, onSelectSession, onNewSession, initialSessionId, skipInitialProjectSelection, onInitialRestoreDone, refreshKey, onSessionDeleted, selectedCwd: selectedCwdProp, onCwdChange, onOpenFile, explorerRefreshKey, onExplorerRefresh, onAtMention, onAtMentions }: Props) {
  ...
  useEffect(() => {
    fetch("/api/home").then((r) => r.json()).then((d: { home?: string; managedSessions?: boolean }) => {
      if (d.home) setHomeDir(d.home);
      setManagedSessions(d.managedSessions === true);
  ...
  const projectRootFor = useCallback((cwd: string | null): string | null => {
    if (!cwd) return null;
    if (managedSessions && homeDir) return homeDir;
  ...
  }, [worktreeState, allSessions, managedSessions, homeDir]);
  ...
  useLayoutEffect(() => {
    // Managed server sessions do not expose Git/worktree operations.
    if (!selectedCwd || managedSessions === true) {
  ...
  }, [selectedCwd, wtRefreshKey, refreshKey, managedSessions]);

  // Auto-select cwd and restore session from URL on first load
  useEffect(() => {
    if (managedSessions === null) return;
    ...
    if (selectedCwd === null) {
      ...
      if (projects.length > 0) {
        ...
        const recentSession = allSessions.find((session) =>
          (session.projectRoot ?? session.cwd) === project
        );
        setSelectedCwd(managedSessions && recentSession ? recentSession.cwd : project);
      }
    }
  }, [allSessions, selectedCwd, initialSessionId, skipInitialProjectSelection, onSelectSession, onInitialRestoreDone, managedSessions]);
  ...
  const handleDefaultCwd = useCallback(async () => {
    try {
      ...
      const data = await res.json() as { cwd?: string; sessionId?: string; error?: string };
      if (data.cwd) {
        ...
        setDropdownOpen(false);
        if (managedSessions) {
          const draftId = data.sessionId
            ?? (typeof crypto.randomUUID === "function" ? crypto.randomUUID() : `${Date.now()}`);
          onNewSession?.(draftId, data.cwd);
        }
      }
    } catch {
      // ignore
    }
  }, [managedSessions, onNewSession]);
  ...
  }, [managedSessions, selectedCwd, onNewSession]);
  ...
  const worktreeGuide = managedSessions === false
    && selectedCwd
    && worktreeState
    && selectedProject === worktreeState.projectRoot
  ...
  const inactiveWorktreeSelector = managedSessions === false
    ? (worktreeGuide
      ...
      <div className="sidebar-session-list" style={{ flex: explorerOpen && (selectedCwdProp || selectedCwd) ? "1 1 0" : "1 1 auto", overflowY: "auto", padding: "0", minHeight: 80 }}>
        ...
        {sessionTree.map((node) => (
          <SessionTreeItem
            key={node.session.id}
            node={node}
            managedSessions={managedSessions === true}
            ...
            onSessionDeleted={(id) => {
              if (managedSessions && id === selectedSessionId) setSelectedCwd(null);
              onSessionDeleted?.(id);
              loadSessions();
            }}
            depth={0}
      ...
      {confirmDelete ? (
        ...
        <>
          <div style={{ flex: 1, minWidth: 0, fontSize: 12, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {managedSessions ? "Delete session and workspace " : "Delete "}<span style={{ fontWeight: 600 }}>&ldquo;{title.slice(0, 22)}{title.length > 22 ? "…" : ""}&rdquo;</span>?
          </div>
