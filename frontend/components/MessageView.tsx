...
export const MessageView = memo(function MessageView({ message, isStreaming, toolResults, modelNames, cwd, onOpenFile, entryId, onFork, forking, onNavigate, prevAssistantEntryId, onEditContent, showTimestamp, prevTimestamp, sessionId }: Props) {
  ...
  if (message.role === "custom") {
    ...
    // Artifact publication remains active in the backend and in the Agent's
    // presentation protocol, but its experimental duplicate card is not part
    // of the production conversation UI.
    if (customMessage.customType === "自动发布产物") return null;
      ...
      >
        ...
        {contentExpanded ? (
          <div style={{ padding: "6px 9px" }}>
            ...
            {text ? <MarkdownBody className="markdown-custom-message" cwd={cwd} onOpenFile={onOpenFile} sessionId={sessionId}>{text}</MarkdownBody> : <span style={{ color: "var(--text-dim)", fontSize: 12 }}>(no message)</span>}
          </div>
        ) : (
          <button
...
function BashExecutionView({ message, sessionId }: { message: BashExecutionMessage; sessionId?: string }) {
  ...
  const fullOutputUrl = sessionId && message.fullOutputPath
    ? apiUrl(`/api/agent/${encodeURIComponent(sessionId)}/bash-output?path=${encodeURIComponent(message.fullOutputPath)}`)
    : null;
