...
export function AppShell() {
  ...
  const [managedSessions, setManagedSessions] = useState<boolean | null>(null);
  const managedDraftCwdRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    void fetch("/api/home")
      .then((response) => response.json() as Promise<{ managedSessions?: boolean }>)
      .then((data) => setManagedSessions(data.managedSessions === true))
      .catch(() => setManagedSessions(false));
  }, []);

  // Also discard an untouched draft when it is replaced by another draft or
  // the user returns to an existing conversation without closing the page.
  useEffect(() => {
    const nextDraft = managedSessions === true ? newSessionCwd : null;
    const previousDraft = managedDraftCwdRef.current;
    managedDraftCwdRef.current = nextDraft;
    if (!previousDraft || previousDraft === nextDraft) return;

    const body = new URLSearchParams({ cwd: previousDraft });
    void fetch("/api/default-cwd/discard", { method: "POST", body }).catch(() => {});
  }, [managedSessions, newSessionCwd]);

  // Managed mode allocates an isolated workspace before the first prompt so
  // uploads and the file explorer are immediately available. If the browser is
  // closed while that workspace is still an untouched draft, ask the backend
  // to remove it. The backend re-checks that it is empty and has no live Agent.
  useEffect(() => {
    if (managedSessions !== true || !newSessionCwd) return;

    const discardDraft = (event: PageTransitionEvent) => {
      if (event.persisted) return;
      const url = apiUrl("/api/default-cwd/discard");
      const body = new URLSearchParams({ cwd: newSessionCwd });
  ...
  const handleKeyboardNewSession = useCallback(async (cwd: string) => {
    if (!managedSessions) {
    ...
    try {
      ...
      const data = await response.json() as { cwd?: string; sessionId?: string; error?: string };
      if (!response.ok || !data.cwd) {
        throw new Error(data.error ?? `HTTP ${response.status}`);
      }
      handleNewSession(data.sessionId ?? `kb-${Date.now()}`, data.cwd);
    } catch (error) {
      console.error("Failed to allocate managed session workspace", error);
    }
  }, [managedSessions, handleNewSession]);

  // Global keyboard shortcuts (handles Esc, Ctrl+Alt+N etc.)
  useGlobalKeyboardShortcuts({
    onNewSession: (cwd: string) => { void handleKeyboardNewSession(cwd); },
    activeCwd: managedSessions === null ? null : activeCwd,
  });
  ...
  const handleSessionDeleted = useCallback((sessionId: string) => {
    ...
    if (selectedSession?.id === sessionId) {
      ...
      setSelectedSession(null);
      setNewSessionCwd(managedSessions ? null : cwd ?? null);
      setSessionKey((k) => k + 1);
      setBranchTree([]);
      setBranchActiveLeafId(null);
      setSystemPrompt(null);
  ...
  }, [selectedSession, router, managedSessions]);
