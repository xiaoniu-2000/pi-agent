...
test("configures HTTP_PROXY, HTTPS_PROXY, and NO_PROXY for global fetch", async (t) => {
  ...
  const usedHttpConnect = connectTargets.includes("target.invalid:80");
  const usedForwardProxy = forwardedRequests.some((line) =>
    line === "GET http://target.invalid/through-http-proxy"
  );
  assert.equal(usedHttpConnect || usedForwardProxy, true);
  if (usedHttpConnect) {
  ...
  assert.equal(connectTargets.includes("target.invalid:443"), true);
