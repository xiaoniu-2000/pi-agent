...
import {
  buildArtifactPublicationMarkdown,
  findChangedWorkspaceArtifacts,
...
const RICH_OUTPUT_MARKER = "[pi-web-rich-output-v2]";
const RICH_OUTPUT_SYSTEM_PROMPT = `

${RICH_OUTPUT_MARKER}
This is the complete and authoritative Pi Web presentation protocol. Never inspect or modify Pi Web frontend/backend source code, routes, or configuration to learn how to present results.
Pi Web automatically detects new or modified user-facing files in the current session workspace after every run, embeds result images, and supplies download controls. In your final response, briefly describe the artifacts you created, but do not inspect application source or construct localhost, /api/files, or guessed download URLs. Create deliverables only inside the current session workspace.
The web client can also render interactive Apache ECharts charts. For time series, comparisons, distributions, or other suitable structured data, include an interactive chart by emitting a fenced code block whose language is \`echarts\`. Its body must be one strict JSON object containing a valid ECharts option: no JavaScript functions, comments, or trailing commas. Prefer ECharts in addition to downloadable result files when an interactive chart improves the answer.`;
...
export class AgentSessionWrapper {
  ...
  private applyForcedEmptySystemPrompt(): void {
    ...
    if (
      process.env.PI_WEB_RICH_OUTPUT !== "0"
      && this.inner.agent.state?.systemPrompt
      && !this.inner.agent.state.systemPrompt.includes(RICH_OUTPUT_MARKER)
    ) {
      this.inner.agent.state.systemPrompt += RICH_OUTPUT_SYSTEM_PROMPT;
    }
  }
  ...
  private beginArtifactTracking(): void {
    ...
    this.artifactSnapshot = snapshotWorkspaceArtifacts(this.cwd);
  }

  private publishChangedArtifacts(): void {
    const before = this.artifactSnapshot;
    this.artifactSnapshot = null;
    if (!before) return;

    const files = findChangedWorkspaceArtifacts(before, this.cwd);
    const content = buildArtifactPublicationMarkdown(files, this.cwd);
    if (!content) return;
        ...
        if (managedSessionFromFile(currentSessionFile, this.userId)) {
          ...
          try {
            ...
            if (!entry.parentId) {
              ...
              temporaryForkFile = sourceManager.createBranchedSession(entry.parentId);
              const branchHeader = sourceManager.getHeader();
              if (!branchHeader) throw new Error("Failed to create forked session header");
              header = {
                ...branchHeader,
                id: target.sessionId,
                timestamp,
            ...
            const newSessionFile = join(
              target.sessionRoot,
              `${fileTimestamp}_${target.sessionId}.jsonl`,
            );
            const serialized = [header, ...entries]
              .map((item) => JSON.stringify(item))
              .join("\n");
            writeFileSync(newSessionFile, `${serialized}\n`, {
              encoding: "utf8",
              flag: "wx",
              mode: 0o640,
            });
            ...
            removeManagedSessionRoot(target);
            throw error;
          }
        }
  ...
  const starting = (async () => {
    ...
    const sessionManager = sessionFile
      ...
      : SessionManager.create(
          ...
          newSession?.sessionId ? { id: newSession.sessionId } : undefined,
        );
