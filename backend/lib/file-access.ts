...
export async function getAllowedFileRoots(userId?: string): Promise<Set<string>> {
  ...
  for (const s of sessions) {
    ...
    // Managed mode uses the user root only as a UI grouping key; do not expose
    // transcript/meta files above each session workspace through /api/files.
    if (s.projectRoot && !managedSessions) roots.add(normalizeSlashes(s.projectRoot));
  }

  if (!managedSessions) {
    // Also allow ~/pi-cwd-* directories created by the legacy default-cwd endpoint.
    try {
